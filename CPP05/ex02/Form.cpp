...
/*   Updated: 2022/08/04 20:53:13 by vmiseiki         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
Form::Form(std::string name, int const req, int const exe) : _name(name), _sigreq(req), _exereq(exe), _signed(false)
{
	try 
	{
		if (req < 1 || exe < 1)
...
bool Form::beExecuted(Bureaucrat const &src) const
{
	try 
	{
		if (!getSigStatus())
			throw (FormNotSigned());
		if (src.getGrade() > getExeReq())
			throw (GradeTooLowException());
		return(true);
	}
	catch (std::exception &err)
